...
export class EquipmentEditComponent implements OnInit {
  ...
  private initForm() {
    ...
    if (this.editMode) {
      const buff = this.buffService.getBuffEquipment(this.id);
      buffName = buff.name;
      bufftype = buff.type;
      dexterityMod = buff.dexterityMod;
    ...
    this.buffForm = new FormGroup({
      ...
      dexterityMod: new FormControl(dexterityMod, Validators.required),
  ...
  onSubmit() {
    ...
    let newId;
    if (this.editMode) {
      newId = this.buffService.getBuffEquipment(this.id).id;
    } else {
      newId = RandomId();
    }
    const newBuff = new Equipment(
      this.buffForm.value.name,
      newId,
      this.buffForm.value.dexterityMod,
      this.buffForm.value.type,
      this.buffForm.value.valueBuffs,
      this.buffForm.value.notes
    );
    if (this.editMode) {
      this.buffService.updateBuffEquipment(this.id, newBuff);
    } else {
      this.buffService.addBuffEquipment(newBuff);
    }
    this.onCancel();
  }
